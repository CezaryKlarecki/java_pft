...
public class ContractCreationTests extends TestBase {
  ...
  public Iterator<Object[]> validContractsFromJson() throws IOException {
    List<Object[]> list = new ArrayList<Object[]>();
    try(BufferedReader reader = new BufferedReader(new FileReader(new File("src/test/resources/contracts.json")))) {
      String json = "";
      String line = reader.readLine();
      while (line != null) {
        json += line;
      ...
      Gson gson = new Gson();
      List<ContractData> contracts = gson.fromJson(json, new TypeToken<List<ContractData>>() {
      }.getType());
      return contracts.stream().map((c) -> new Object[]{c}).collect(Collectors.toList()).iterator();
    }
  }
